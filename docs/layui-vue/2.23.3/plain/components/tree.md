...
使用 `lay-tree` 标签, 创建树形组件, @node-click 监听节点点击。
...
使用 `showCheckbox` 属性开启复选框, `checkedKeys` 属性设置选中项。
...
通过 `checkStrictly` 属性禁用父子关联选择, 让每个复选框都是独立的。
...
使用 `replaceFields` 替换 `data` 中的字段名
