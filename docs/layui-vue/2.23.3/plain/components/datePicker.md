...
将 `type` 设置成 `"year"` 使用年份选择模式。可以用 `year-page` 指定每页展示年份的数量。
...
设置 `simple` 属性后，只需要点击一次后自动关闭，无需点击确认按钮。 对于 `datetime` 、 `time` 与对应 `range` 模式因交互影响无效。
...
仅在 type 等于`date`、`datetime`时有效 传入的一个 Unix 时间戳 (13 位数字，从 1970 年 1 月 1 日 UTC 午夜开始所经过的毫秒数)
...
通过 `disabled-date` 可禁止任何日期，优先级大于 `max` `min`。
...
`default-value` 可用于在 `modelValue` 为空时，面板展示的默认日期。 在 `range` 模式时，会设置 `左侧` 面板默认日期。
...
`format` 用于格式化输出 `modelValue`，`inputFormat` 用于格式化输入框显示。
