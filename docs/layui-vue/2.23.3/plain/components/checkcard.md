...
最常用的选项卡示例，包括`头像`，`标题`，`描述`等部分，可被选择。
...
使用分组`lay-checkcard-group`时, 默认为多选模式。
...
当然你也可以通过 `style` 或者 `class` 自定义卡片大小。
...
通过配置 `v-model` 为 `true` 来配置选项默认被选中。
...
通过仅仅配置 `cover` 属性或者  `cover` 插槽 可以让你的选项成为一个纯图片选项。
...
`lay-checkcard-group`与`lay-checkcard`搭配使用。
