...
通过 `size` 属性, 创建指定尺寸的按钮, 可选值 `lg` `sm` `xs`。
...
使用 `lay-button-group` 标签, 配合 `lay-button` 标签, 创建按钮组。
...
使用 `lay-icon` 标签, 配合 `lay-button` 标签, 创建图标按钮。
...
尽管按钮在同节点并排时会自动拉开间距，但在按钮太多的情况，效果并不是很美好。因此你需要用到按钮容器。
...
使用 `lay-button` 组件，配合 `lay-dropdown` 系列组件，完成下拉菜单。
