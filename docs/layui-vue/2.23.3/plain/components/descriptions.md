...
使用 `lay-descriptions` 与 `lay-descriptions-item` 结合使用。
...
`horizontal` 垂直、 `vertical` 水平 默认 `horizontal`。
...
使用 `align` 内容对齐、 `label-align` label对齐。
...
使用 `style` | `class` 内容样式、 `label-style` | `label-class` label样式。
...
使用 `data-list` 可快速生成描述列表、对象中可传 `descriptions-item` 的所有 `props`
