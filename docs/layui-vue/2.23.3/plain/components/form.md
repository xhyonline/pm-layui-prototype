...
通过 `lay-form` 与 `lay-form-item` 标签，创建 `form` 组件。
...
通过 `mode` 属性 `inline` 值，采用行内布局，默认为 `block` 块状布局。
...
通过 `isLabelTooltip` 属性开启超出隐藏，默认值为 `false`
...
通过 `label-position` 属性改变标签位置，默认值为 `right`，可选值为 `top`，`left`。
...
## 关于 async-validator 的使用


查看： https://github.com/yiminghe/async-validator。


翻译： https://www.cnblogs.com/wozho/p/10955525.html。


## async-validator 基本校验类型
...
## async-validator 中 validator 参数使用
...
## async-validator 中 asyncValidator 参数使用
...
## form-item 配置 async-validator 的使用
