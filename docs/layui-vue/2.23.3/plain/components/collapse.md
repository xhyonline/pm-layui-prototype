...
使用 `lay-collapse` 与 `lay-collapse-item` 标签，创建折叠卡片
...
通过 `accordion` 属性，启用手风琴效果，默认为 `false`。
...
通过 `lay-collapse-item` 标签 `disabled` 属性，控制某项不可展开。
...
通过 `lay-collapse-item` 标签 `title` 属性，自定义标题。
...
通过 `lay-collapse` 标签 `collapseTransition` 属性，控制折叠动画，默认为 `true`。
...
通过 `lay-collapse` 标签 `change` 事件，监听 `item` 折叠。
