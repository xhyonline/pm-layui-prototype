...
使用 `lay-popconfirm` 标签，创建确认提示，使用 `content` 属性设置文本信息，使用 `@confirm` 与 `@cancel` 接收回调事件。
...
通过 `position` 属性设置 `popconfirm` 面板显示位置，默认为 `top` 值，`left` `bottom` `right` 为可选值。
...
通过 `trigger` 属性设置 popconfirm 触发方式，默认为 `hover` 方式，`click` `contextMenu` `focus` 为可选值。
...
通过 `disabled` 属性禁用 `popconfirm` 提示。
