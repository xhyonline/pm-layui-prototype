...
如 Message/Confirm/Loading/Page/Iframe/Drawer/Notify/Photos，应有尽有。
...
若你希望使用 `layer-vue` 搭配其他组件库，仅仅需要安装 `@layui/layer-vue` 包。
...
若在 `layui-vue` 环境使用，可忽略 `app.use(layer)` 步骤。
...
通过 layer.msg(content, options) 创建消息窗, content 为消息内容, options 为可选配置。
...
通过 layer.confirm(content, options) 创建确认框, content 为消息内容, options 为可选配置。
...
通过 layer.notify(options) 创建通知消息。
...
通过 layer.photos(options) 创建图片预览弹层, 参数 `options` 主要传入预览的图片链接。
...
通过 layer.load(type, options) 创建加载层, 第一个参数 `type` 为加载动画样式, 第二个参数 `options` 为可选配置。
...
通过 layer.open(option) 创建模态窗, 目前支持 `iframe` `page` 等类型, 你可以使用 options 选项开启 `resize` `offset` 等更多特性。
...
模态窗除 `layer.open` 的函数调用方式外，我们同时提供 `template` 的使用方式。
...
通过 layer.drawer(options) 创建抽屉层, 参数 `options` 为可选配置。 抽屉本质上是一个特殊的模态窗, 你可以使用 `offset` 选项来打开方向。
...
我们提供 `teleport` 指定弹层父级元素，通过 `teleportDisabled` 属性禁用模态模式，默认为 `false`。
...
通过 `layer.open(options)` 创建模态窗, 通过 `layer.close(id)` 关闭指定模态窗，通过 `layer.closeAll()` 关闭所有模态窗。
...
你可以通过 `success` `end` `close` `beforeClose` `full` `min` `restore` `revert` 等回调属性，监听 layer 的生命周期。
...
全部的封装都带有可选属性 `option`，**调用时与内部预置的 `defaultOption` 配置混合，`Option` 的配置将有更高的优先级**。
...
以下是 layer 支持的基础属性 `LayerProps`。根据 _type_ 的不同，对基础属性的扩充也各不同，`(通用属性 + 回调) + 可选属性 = LayerProps`。
...
以下是 `BtnType`，为 `LayerProps` 的 `btn` 属性所用。
