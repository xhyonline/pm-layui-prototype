...
Json-schema-form 是在 form 基础上封装的高级组件。


通过简单的 JSON 配置来定义表单的结构和验证规则，从而实现动态生成表单的功能。


## 安装


该组件不内置在 layui-vue 中，需要单独安装。
...
当您使用 `@layui/unplugin-vue-components/resolvers` 默认注册 `layui-vue` 所有组件时，需要排除 `LayJsonSchemaForm。`
...
使用 `lay-json-schema-form` 标签, 创建一个 JSON 表单。内置包含 `input` `textarea` `select` `switch` `rate` `radio` `checkbox` `date` 通用 type 类型
...
使用 `required` 参数设置全部 form-item 校验
