...
通过 <code>config-provider</code> 组件 <code>locale</code> 属性设置启用的语言。
...
在 layui-vue 内部维护了一个 <code>vue-i18n</code> 实例, 你可以通过 <code>useI18n</code> 方法来获取。


通过 locales 属性扩展语言包, 自定义语言优先级大于组件库内部维护的语言。
